...
\subsection{IA generativa}
    ...
    Los Grandes Modelos de Lenguaje procesan el lenguaje mediante tokens (unidades de texto inferiores a las palabras). Usan el mecanismo de atención para enfocarse en partes del texto dinámicamente, extratendo el contexto y las relaciones entre palabras.

    Los modelos entrenados con grandes conjuntos de datos sirven como punto de partida para realizar modelos especializados. A estos se les llama modelos fundacionales.

    La ingeniería de prompts se centra en el diseño de instrucciones que se proporcionan a los modelos para obtener respuestas mas específicas, precisas y útiles. Se usan estrategias como el \textbf{Few-shot} donde se incluyen ejemplos, o el \textbf{Zero-shot} donde no se proporcionan ejemplos y se dan descripciones detalladas de la tarea.

    El RAG o \textbf{Retrieval Augmented Generation} busca optimizar la ventana de contexto del modelo. combinando la generacin con la recuperacin de informacin de la Base de Datos previamente indexada. Es un ciclo de rfbfón reduce su tamaño y mejora la velocidad, disminuye el coste de procesamiento, a costa de comprometer la calidad de las respuestas.

    Se pueden elegir mnodelos de lenguaje evaluando su rendimiento con distintos benchmarks.

    Los modelos actuales pueden trabajar con distintos modos de entrada y salida. Combinando texto, imágenes o audio. EStas capacidades les abren la puerta a usarse como agentes autónomos. Optimizando el proceso de backend y otras tareas automáticas.

    De acuerdo con el plan de Digitalización de las Administraciones Públicas. Se da impulso al Datalake de la AGE con la creación de la Plataforma del Dato. Esta platadorma se usa con herramientas de analítica avanzada para entrenar modelos de lenguaje.
