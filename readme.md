# Oposiciones A1

Información general sobre el funcionariado en [Este vídeo](https://www.youtube.com/watch?v=76KDt0VeXyY&t=100s)

- [Oposiciones A1](#oposiciones-a1)
- [Formato del proyecto](#formato-del-proyecto)
  - [Numeración y versiones.](#numeración-y-versiones)
  - [Procesos](#procesos)
    - [Tema nuevo](#tema-nuevo)
    - [Fin de tema](#fin-de-tema)
- [Ediciones anteriores](#ediciones-anteriores)
  - [2025](#2025)
    - [Fase de oposición](#fase-de-oposición)
      - [Primer ejercicio](#primer-ejercicio)
      - [Segundo ejercicio](#segundo-ejercicio)
      - [Tercer ejercicio](#tercer-ejercicio)
    - [Fase de concurso](#fase-de-concurso)
    - [Curso selectivo](#curso-selectivo)
- [Plan de acción](#plan-de-acción)
  - [Estado de los apuntes](#estado-de-los-apuntes)
- [Preguntas pendientes](#preguntas-pendientes)
  - [Meta](#meta)
  - [De los apuntes](#de-los-apuntes)
- [Tareas pendientes](#tareas-pendientes)


# Formato del proyecto
El proyecto se usa para generar un pdf de apuntes con todas las notas que voy generando, ordenadas de la mejor manera posible.


Localizacin | propósito
---|---
**Apuntes** | contiene material externo organizado por temas.
**dist-online** | es un compendio de examenes ofline
**Material Bruto** | contiene documentacin no especfica a una fuente de apuntes o a un tema especfico. Puede ser conocimiento genérico, leyes, o documentación específica de las oposiciones.
**Tex** | Las fuentes para generar mi propia documentación y apuntes.

## Numeración y versiones.
Se opta por doble numeración, relativa al proyecto y a los apuntes. Dándole la misma importancia a ambos De esta manera, una primera parte son los tradicionales major, minor, patch. Y una segunda parte enfocada a la cantidad de temas y revisiones realizadas. Un primer número con la cantidad de temas completados, y un segundo número de revisiones sobre esa versión. Permitiendo tener mayor control sobre las versiones de los apuntes sin cambiar las versiones del proyecto, y viceversa.

Por ejemplo, la versión 1.0.1-1.0 implica 1.0.1 para el proyecto en sí, y un tema hecho sin mas revisiones.
## Procesos

### Tema nuevo
- Preparar changelog
- Generar nuevo tex y añadirlo al index

### Fin de tema
- Añadir cambios al changelog
- marcar el programa
- generar y subir etiqueta y versión
- borrar WIP

# Ediciones anteriores
Listado de convocatorias anteriores, por referencia.

## 2025
[2025](https://boe.es/boe/dias/2025/12/18/pdfs/BOE-A-2025-25938.pdf)

El proceso de 2025 contó con 290 plazas. Para todo el proceso se publicarán los baremos con anterioridad a la realización de la prueba. Cada ejercicio tiene una puntuación mínima de **al menos** un 35% del máximo. La calificación total será la suma de las anteriores.
En caso de obtención de un mínimo del 50% de la nota de los ejercicios. se coservará para la convocatoria inmediatamente siguiente, si el contenido es análogo.

### Fase de oposición

#### Primer ejercicio
Este proceso incluye una primera fase eliminatoria con un cuestionario tipo test de 130 preguntas en dos partes. Las incorectas restan un tercio del valor. Todas las preguntas valen igual. puntúa 0~60 con un mínimo de 30

Una parte de hasta 100 preguntas de **Tema específico** (anexo B) y 5 preguntas adicionales en caso de anulación de alguna anterior. y una segunda parte de hasta 30 preguntas con tres adicionales sobre **Temas generales** (Anexo A). Para las dos partes hay dos horas.

#### Segundo ejercicio
Tabién cuenta con dos partes. Puntúa de la siguiente manera

parte   | puntuación | corte
--------|------------|---
primera | 0~50       | 25
segunda | 0~30       | 15
total   | 0~80       | 40

La primera parte es la resolución de un supuesto práctico entre varios supuestos tecnológicos. Se valora la capacidad de análisis y de expresión oral y escrita. Se dispone de 4 horas.

Una segunda parte con un anális pormenorizado relativo al supuesto de la primera parte. En cuanto al marco jurídico y de contexto. Teniendo especial cuenta al enlace con los temas generales. Se dispone de una hora. 

#### Tercer ejercicio
Eliminatorio de inglés. Se puede acreditar un B2 o superior. El título debe estr contemplado en el anexo IV con máximo 5 años de antiguedad. Se valor como apto o no apto, sin valorar puntuación alguna.

Si no se puede acreditar en vigor, hay una prueba de nivel de comprensión oral y escrita. La parte oral es un cuestionario de hasta 100 preguntas e 45 minutos con 7~10 segundos entre cada una. Para la parte escrita un cuestionario de hasta 100 preguntas en 75 minutos.

### Fase de concurso
Solo para promoción interna.

### Curso selectivo
Curso con el objeto de adquirir conocimiento y habilidades para la preparación específica. Se valora con 70 puntos necesitando obtener 35. De no aprobarlo, se queda exento del proceso de oposición en la siguiente convocatoria, siempre que el contenido sea análogo.




# Plan de acción
El plan es que no hay plan. De momento estoy en una fase de recapitulación. Quiero ver cómo está el patio. Cómo va a ser enfrentarse a esto...

- Planteamiento de las oposiciones
  - Crear calendario
  - Plantear temario
- Estudiar
  - Elaboración de apuntes
  - Elaboración de preguntas tipo
- Aprobar
  - Poner velas
- Contingencias
  - Planes para llevar (o no) las oposiciones en caso de que ocurran cosas en mi vida

## Estado de los apuntes
_O cómo vamos con todo este lío_

Tema | Apuntes  | Notas
----:|----------|-------
1    | Hecho


# Preguntas pendientes
## Meta
- ¿Con qué frecuencia se anulan preguntas?
- Tasa de aprobados del curso selectivo.

## De los apuntes
Tema 1, L:225: Qué hace una comisión?

# Tareas pendientes
- Decidir si me mto a esto o no.
- Obtener y organizar apuntes
  - Teoría y examenes (si es posible)
- Crear alerta para encontrar la próxima convcatoria
- Comprender el funcionamiento de las leyes de la unión europea.
- Crear registro offline de leyes y textos legales.